...
import useAxiosSecure from "../../Hooks/useAxiosSecure";
import useAuth from "../../Hooks/useAuth";
import { FaStar, FaShoppingCart, FaHeart, FaRegHeart, FaClock, FaTruck } from "react-icons/fa";
import Swal from "sweetalert2";
import Loading from "../../Components/Loading/Loading";
...
const MealsDetails = () => {
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 max-w-6xl">
        {/* Top Section: Meal Info */}
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col lg:flex-row border border-yellow-100">
          {/* Image Side */}
          <div className="lg:w-1/2 relative h-96 lg:h-auto">
            <img
              src={meal.FoodImage}
            ...
            <div className="absolute top-4 left-4">
              <button
                onClick={handleAddToFavorite}
                className={`btn btn-circle bg-white/80 hover:bg-white border-none shadow-lg text-xl ${favoriteStatus.isFavorite ? "text-red-500" : "text-yellow-400"
                  }`}
                title={favoriteStatus.isFavorite ? "Already in favorites" : "Add to favorites"}
              >
                {favoriteStatus.isFavorite ? <FaHeart /> : <FaRegHeart />}
          ...
          <div className="lg:w-1/2 p-8 lg:p-12 flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-2">
              <span className="bg-yellow-100 text-yellow-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
                Chef ID: {meal.ChefId}
              </span>
            ...
            <div className="flex flex-wrap gap-3">
              {meal.ingredients ? (
                meal.ingredients.map((ing, i) => (
                  <span
                    key={i}
                    className="badge badge-lg p-4 bg-gradient-to-r from-yellow-50 to-white text-gray-700 border border-yellow-200 shadow-sm"
                  >
                    {ing}
                  </span>
                ))
                ...
                <>
                  <span className="badge badge-lg p-4 bg-gradient-to-r from-yellow-50 to-white text-gray-700 border border-yellow-200">
                    Fresh Spices
                  </span>
                  <span className="badge badge-lg p-4 bg-gradient-to-r from-yellow-50 to-white text-gray-700 border border-yellow-200">
                    Premium Meat
          ...
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-yellow-100">
            ...
            <div className="flex flex-wrap gap-3">
              {meal.DeliveryArea?.map((area, index) => (
                <span
                  key={index}
                  className="px-4 py-2 bg-gradient-to-r from-yellow-50 to-white text-yellow-700 rounded-lg font-semibold text-sm border border-yellow-200 shadow-sm flex items-center gap-2"
                >
                  <span className="text-yellow-500">📍</span> {area}
                </span>
              )) || (
                <>
                  <span className="px-4 py-2 bg-gradient-to-r from-yellow-50 to-white text-yellow-700 rounded-lg font-semibold text-sm border border-yellow-200">
                    📍 Dhaka Central
                  </span>
                  <span className="px-4 py-2 bg-gradient-to-r from-yellow-50 to-white text-yellow-700 rounded-lg font-semibold text-sm border border-yellow-200">
                    📍 Gulshan Area
        ...
        {/* Review Section */}
        <div className="mt-16 bg-white rounded-3xl shadow-xl p-8 lg:p-12 border border-yellow-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-900">
                Customer Reviews
            ...
            <div className="bg-yellow-50 px-6 py-3 rounded-2xl flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-yellow-700 font-bold mt-1">
                  Overall Rating
                </span>
              </div>
            </div>
          ...
          <div className="space-y-6 mb-12">
            {reviews.length > 0 ? (
              reviews.map((review, idx) => (
                <div
                  key={idx}
                  className="flex gap-4 p-6 bg-gradient-to-r from-white to-yellow-50 rounded-2xl border border-yellow-100"
                >
                  <img
                    src={
                      review.reviewerImage ||
                    ...
                    <div className="rating rating-xs my-1">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <input
                          key={s}
                          type="radio"
                          className={`mask mask-star-2 ${s <= review.rating ? "bg-yellow-400" : "bg-gray-300"
                            }`}
                          checked
                          readOnly
                        />
              ...
              <div className="text-center py-10 bg-gradient-to-r from-white to-yellow-50 rounded-2xl border-dashed border-2 border-yellow-200">
                <p className="text-gray-500 italic">
                  No reviews yet. Be the first to taste and tell!
                </p>
              </div>
          ...
          {/* Add Review Form */}
          <div className="border-t border-yellow-200 pt-10">
            <h3 className="text-xl font-bold text-gray-900 mb-6">
              Leave a Review
            </h3>
            <form onSubmit={handleReviewSubmit} className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  name="comment"
                  className="textarea textarea-bordered w-full h-32 rounded-xl focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 border-yellow-200"
                  placeholder="Tell us what you loved about this meal..."
                  required
                ></textarea>
              </div>
              <button
                type="submit"
                className="btn bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white rounded-xl px-8 shadow-lg hover:shadow-xl"
              >
                Submit Review
              </button>
            </form>
